...
{
	cameras.AddCamera(std::make_unique<Camera>(
	wnd.Gfx(), "A", dx::XMFLOAT3{ -13.5f, 6.f, 3.5f }, 0.f, PI / 2.f
		));
	...
	wnd.mouse.OnWheelUp_DELEGATED.Assign([&](int, int) {cameras->AddTravelSpped(2.f); });
	wnd.mouse.OnWheelDown_DELEGATED.Assign([&](int, int) {cameras->AddTravelSpped(-2.f); });
}
