...
{
public:
	static IndexedTriangleList MakeTesselated(VertexSystem::VertexLayout layout, int latDiv, int longDiv)
	{
		...
		VertexSystem::VertexBuffer vb{ std::move(layout) };
	...
	static IndexedTriangleList Make(std::optional<VertexSystem::VertexLayout> layout = std::nullopt)
	{
		using Element = VertexSystem::VertexLayout::ElementType;
		if (!layout)
			layout = VertexSystem::VertexLayout{}.Append(Element::Position3D);
