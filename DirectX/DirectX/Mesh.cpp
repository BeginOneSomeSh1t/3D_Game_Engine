...
Node::Node(int id, const std::string& name, std::vector<Mesh*> meshPtrs_in, const DirectX::XMMATRIX& transform_in) noxnd
	:
	id(id),
	meshPtrs(std::move(meshPtrs_in)),
	name(name)
{
...
std::unique_ptr<Mesh> Model::ParseMesh(Graphics& gfx, const aiMesh& mesh, const aiMaterial* const* pMaterials)
{
	...
	else
	{
		...
		{
			float specularIntensity = .8f;
			float specularPower;
			float padding[2];
		} pmc;
		pmc.specularPower = shininess;
		bindablePtrs.push_back(PixelConstBuffer<PSMaterialConstant>::Resolve(gfx, pmc, 1u));
	}
