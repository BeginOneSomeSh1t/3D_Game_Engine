#include "PixelShader.h"
#include "GraphicsThrowMacros.h"
#include "ConversionMacros.h"
#include "BindableCodex.h"

namespace Bind
{
	PixelShader::PixelShader(Graphics& gfx, const std::string& path)
		:
		path(path)
	{
		INFOMAN(gfx);

		Microsoft::WRL::ComPtr<ID3DBlob> pBlob;
		GFX_THROW_INFO(D3DReadFileToBlob(std::wstring{path.begin(), path.end()}.c_str(), &pBlob));
		GFX_THROW_INFO(GetDevice(gfx)->CreatePixelShader(
			pBlob->GetBufferPointer(),
			pBlob->GetBufferSize(),
			nullptr,
			&pPixelShader
		));

	}

	void PixelShader::Bind(Graphics& gfx) noexcept
	{
		GetContext(gfx)->PSSetShader(
			pPixelShader.Get(), nullptr, 0u
		);
	}
	DEFINE_RESOLVE(PixelShader, PixelShader, Graphics& gfx, const std::string& path)
	{
		return Codex::Resolve<PixelShader>(gfx, path);
	}
	DEFINE_GENERATEUID(PixelShader, const std::string& path)
	{
		using namespace std::string_literals;
		return typeid(PixelShader).name() + "#"s + path;
	}
	DEFINE_OVERIDDEN_GETUID(PixelShader)
	{
		return GenerateUID(path);
	}

}